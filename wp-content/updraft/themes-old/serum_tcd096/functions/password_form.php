...
function pw_meta_box() {
	...
	add_meta_box(
		...
		__( 'Contents to encourage member registration', 'tcd-serum' ), // label
...
<div class="tcd_custom_fields">
 ...
 <div class="tcd_cf_content">
  <select name="pw_content">
   <option value=""><?php _e( 'Select a content to encourage member registration.', 'tcd-serum' ); ?></option>
   <?php for ( $i = 1; $i <= 3; $i++ ) : ?>
   <option value="<?php echo $i; ?>" <?php selected( $i, $post->pw_content ); ?>><?php echo tcd_admin_label('content') . $i . ' : ' . esc_html( $options['pw_name' . $i] ); ?></option>
   <?php endfor; ?>
  </select>
 </div><!-- END .content -->
...
function tcd_password_form() {
	...
	if ( ! $pw_content ) {
		...
		if ( $options['pw_label' . $pw_content] ) { 
			$output .= '<label class="c-pw__box-label" for="' . $label . '">' . esc_html( $options['pw_label' . $pw_content] ) . '</label>';
		}

		$output .= '<input class="c-pw__box-input" name="post_password" id="' . $label . '" type="password" size="20"><input class="c-pw__btn c-pw__btn--submit" type="submit" name="Submit" value="' . esc_attr( $options['pw_password_button_label' . $pw_content] ) . '"></div>';
