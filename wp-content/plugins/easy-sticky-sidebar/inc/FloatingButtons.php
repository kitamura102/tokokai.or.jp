...
/*
 * Floating buttons options
 * @package sticky-sidebar/inc
 * @since 1.4.5
 */
class Easy_Sticky_Sidebar_Floating_Buttons {
    /**
    ...
    public static function get_buttons($stickycta) {
        ...
        // If button rows are missing but per-button styles exist, rebuild rows
        // from style keys so Styling tab is never empty.
        if (empty($floating_buttons) && !empty($button_style)) {
            foreach ($button_style as $style_key => $style_value) {
                $floating_buttons[$style_key] = self::get_data(is_array($style_value) ? $style_value : []);
            }
        }
        ...
        // Final fallback: keep at least one editable style item visible in admin.
        if (empty($new_buttons)) {
            $new_buttons[] = self::get_data([
                'icon' => 'fa-solid fa-phone',
                'text' => __('Button 1', 'easy-sticky-sidebar'),
            ]);
        }
    ...
    public function generate_css($stickycta) {
        ...
        if (!empty($style)) {
            $safe_style = wp_strip_all_tags($style);
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $safe_style is sanitized CSS declarations generated by this plugin.
            printf("%s {\n%s}\n", esc_html($wrapper), $safe_style);
        }

        ob_start();
        Easy_Sticky_Sidebar_Generate_CSS::get_font_style($stickycta->default_font_family);
        $font_style = ob_get_clean();

        if (!empty($font_style)) {
            $safe_font_style = wp_strip_all_tags($font_style);
            // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- $safe_font_style is sanitized CSS declarations generated by this plugin.
            printf("%s li, %s li a {\n%s}\n", esc_html($wrapper), esc_html($wrapper), $safe_font_style);
        }
    ...
    public function style_options($stickycta) {
        if (has_action('easy_sticky_sidebar_floating_buttons_style')) : ?>
            <div class="easy-sticky-sidebar-fieldset ess-settings-panel floating-buttons-options ess-template-floating-only <?php echo esc_attr(Easy_Sticky_Sidebar_Utils::pro_tab_class('easy_sticky_sidebar_floating_buttons_style')); ?>" id="section-floating-button-style-options" data-tab-label="<?php esc_attr_e("Floating Button Options", "easy-sticky-sidebar"); ?>">
            <p class="wordpress-cta-instruction ess-settings-span-2"><?php esc_html_e('If you edit an individual button, that style will override the global style for only that button.', 'easy-sticky-sidebar'); ?></p>
    ...
    public function single_buttons_style_template() {  ?>
        <div class="easy-sticky-sidebar-fieldset easy-sticky-sidebar-fieldset-floating-button ess-settings-subpanel" data-id="{{data.button_no}}">
    ...
    public function buttons_style_options($stickycta) {
        ...
        <div id="floating-single-button-styles">
            <?php foreach ($floating_buttons as $key => $button) :
                ...
                echo wp_kses($single_button_html, $allow_html);
            endforeach; ?>
        </div>
