...
function po_admin_print_scripts() {
	...
	if ('page' === $post_type || 'treatment' === $post_type) {
...
/**
 * Change post order of custom post type 'treatment' and 'company'
 */
function change_post_order() {
	...
	if ( 'page' !== $post_type && 'treatment' !== $post_type) return;
...
function po_pre_get_posts( $query ) {
  ...
  // Modify post order of treatment and company
  //if ( is_admin() || ! $query->is_main_query() ) return;
  if ( is_admin() ) return;
  if ( (is_post_type_archive('treatment') && $query->is_main_query()) ) { 
...
function po_get_previous_post_where( $where, $in_same_term, $excluded_terms, $taxonomy, $post ) {
	if ( in_array( $post->post_type, ['treatment'] ) ) {
